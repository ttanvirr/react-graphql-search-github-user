...
- [Setup Vite, React-ts, tailwindcss and shadcn ui](https://ui.shadcn.com/docs/installation/vite#existing-project)
...
- replace existing theme setup with custom shadcn theme:

```bash
npx shadcn@latest apply --preset b2BoWoOuW
```
...
- The explorer may not be available now.
...
src/components/charts/ForkedRepos.tsx
...
src/components/charts/PopularRepos.tsx
...
src/components/charts/UsedLanguages.tsx
...
- add `chart` shadcn ui (which also installs `recharts` library)
